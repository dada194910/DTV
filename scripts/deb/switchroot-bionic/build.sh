...
./configure --prefix=$CMAKE_PREFIX_PATH --enable-shared --disable-static \
  ...
  --disable-protocols --enable-protocol='file,http,tcp,rtmp,hls,https,httpproxy,tls' \
...
cd /opt
mkdir -p /tmp/deb/DEBIAN /tmp/deb/opt/wiliwili/lib

cmake -B build -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$CMAKE_PREFIX_PATH \
  -DCMAKE_INSTALL_RPATH=$CMAKE_PREFIX_PATH/lib -DCMAKE_INSTALL_SHARE_DIR=/usr/share \
  -DPLATFORM_DESKTOP=ON -DUSE_SYSTEM_CURL=ON -DHOMEBREW_MPV=$CMAKE_PREFIX_PATH \
  -DINSTALL=ON -DCUSTOM_RESOURCES_DIR=$CMAKE_PREFIX_PATH \
  -DGLFW_BUILD_WAYLAND=ON -DGLFW_BUILD_X11=ON
cmake --build build -j$(nproc)
DESTDIR=/tmp/deb cmake --install build

ldd /tmp/deb/opt/wiliwili/bin/wiliwili | grep $CMAKE_PREFIX_PATH | awk '{print $3}' | xargs install -t /tmp/deb/opt/wiliwili/lib
